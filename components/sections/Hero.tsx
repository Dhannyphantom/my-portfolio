...
import { ArrowDown, Download, Mail, Sparkles, Code as Code2, Pill } from 'lucide-react';
...
export function Hero() {
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <div className="absolute bottom-20 right-10 w-96 h-96 bg-cyan-400/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '700ms' }} />

      <div className="absolute top-32 right-20 animate-float">
        <Code2 className="h-12 w-12 text-blue-400/30 dark:text-blue-600/30" />
      </div>
      <div className="absolute bottom-40 left-32 animate-float" style={{ animationDelay: '1s' }}>
        ...
        <div className={`text-center transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-8xl font-bold tracking-tight mb-6">
            <span className="block text-slate-900 dark:text-white mb-2">
              Hi, I'm <span className="bg-gradient-to-r from-blue-600 via-cyan-600 to-blue-600 bg-clip-text text-transparent animate-gradient">Daniel Olojo</span>
            </span>
            <span className="block text-3xl sm:text-4xl lg:text-5xl font-semibold bg-gradient-to-r from-slate-600 to-slate-800 dark:from-slate-300 dark:to-slate-500 bg-clip-text text-transparent">
              Pharmacist Intern & Full Stack Developer
            </span>
          </h1>

          <p className="mt-8 text-xl sm:text-2xl text-slate-600 dark:text-slate-300 max-w-3xl mx-auto leading-relaxed font-light">
            Crafting <span className="font-semibold text-blue-600 dark:text-blue-400">impactful digital solutions</span> that bridge healthcare, technology, and communities through innovation and creativity.
          </p>
